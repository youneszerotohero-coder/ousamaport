  ...
  return (
    <div
      ...
      <div
        ...
        <header
          className="staggered-menu-header absolute inset-0 flex items-center justify-between px-6 bg-transparent pointer-events-none z-20"
              ...
              {items && items.length ? (
                items.map((it, idx) => (
                  <li
                    ...
                    key={it.label + idx}>
                    <a
                      className="sm-panel-item relative text-black font-semibold text-[2rem] sm:text-[3rem] lg:text-[3.5rem] cursor-pointer leading-none tracking-[-1px] uppercase transition-[background,color] duration-150 ease-linear inline-block no-underline pr-[1.4em]"
                      href={it.link}
                      aria-label={it.ariaLabel}
                      data-index={idx + 1}
                      onClick={(e) => {
                        if (onItemClick) {
                          onItemClick(e, it.link);
                        }
                        closeMenu();
                      }}>
                      <span
...
.sm-scope .staggered-menu-header { position: absolute; inset: 0; display: flex; align-items: center; justify-content: space-between; padding: 0 1.5rem; background: transparent; pointer-events: none; z-index: 20; }
...
.sm-scope .staggered-menu-panel { position: fixed; top: 0; right: 0; bottom: 0; width: clamp(260px, 38vw, 420px); height: 100vh; background: #0A0B10; border-left: 1px solid rgba(255, 255, 255, 0.1); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); display: flex; flex-direction: column; padding: 6em 2em 2em 2em; overflow-y: auto; z-index: 10; }
...
.sm-scope .sm-socials-title { margin: 0; font-size: 1rem; font-weight: 500; color: var(--sm-accent, #90EE90); }
...
.sm-scope .sm-socials-link:focus-visible { outline: 2px solid var(--sm-accent, #90EE90); outline-offset: 3px; }
...
.sm-scope .sm-socials-link:hover { color: var(--sm-accent, #90EE90); }
.sm-scope .sm-panel-title { margin: 0; font-size: 1rem; font-weight: 600; color: #fff; text-transform: uppercase; }
.sm-scope .sm-panel-list { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 0.5rem; }
.sm-scope .sm-panel-item { position: relative; color: #ffffff; font-weight: 600; font-size: clamp(2rem, 7vw, 3.5rem); cursor: pointer; line-height: 1; letter-spacing: -1px; text-transform: uppercase; transition: background 0.25s, color 0.25s; display: inline-block; text-decoration: none; padding-right: 1.4em; }
.sm-scope .sm-panel-itemLabel { display: inline-block; will-change: transform; transform-origin: 50% 100%; }
.sm-scope .sm-panel-item:hover { color: var(--sm-accent, #90EE90); }
.sm-scope .sm-panel-list[data-numbering] { counter-reset: smItem; }
.sm-scope .sm-panel-list[data-numbering] .sm-panel-item::after { counter-increment: smItem; content: counter(smItem, decimal-leading-zero); position: absolute; top: 0.1em; right: 0.2em; font-size: 18px; font-weight: 400; color: var(--sm-accent, #90EE90); letter-spacing: 0; pointer-events: none; user-select: none; opacity: var(--sm-num-opacity, 0); }
